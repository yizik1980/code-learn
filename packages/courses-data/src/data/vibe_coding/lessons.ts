...
export const vibeCodingLessons: Lesson[] = [
  {
    ...
    content: [
      ...
      { type: 'text', text: 'זרימת העבודה הבסיסית מורכבת משישה צעדים מחזוריים. שני הצעדים הקריטיים ביותר הם 3 ו-6: תמיד הרץ את הקוד לפני שממשיכים, ותמיד וודא שאתה מבין את מה שנוצר. AI יכול לייצר קוד שעובד אך שאתה לא תבין — ואז תתקשה לתחזק אותו בעתיד.' },
...
6. בדוק שאתה מבין את הקוד שנוצר` },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'text', text: 'פרומפט טוב לקוד בנוי מארבעה חלקים: הקשר (מה הפרויקט וה-stack), מה בדיוק לבנות, דרישות ואילוצים, ודוגמה לפלט הצפוי. דוגמה בתחתית הקוד מציגה זאת בפועל — הקשר מלא, תיאור ברור, סגנון רצוי, וספריה ספציפית לשימוש. שימו לב שכולל "React + TypeScript + Tailwind" מייד מכוון את ה-AI לסגנון נכון.' },
...
השתמש ב-shadcn/ui לכפתורים."` },
      ...
      { type: 'text', text: 'תיקון באגים עם AI דורש מתן מידע מדויק: הקוד הבעייתי, הודעת השגיאה המדויקת, ההתנהגות הצפויה לעומת בפועל. התבנית הזו מייצרת context מלא שמאפשר ל-AI לאתר את הבעיה. שימו לב שהשאלה "למה זה לא עובד?" היא מעורפלת מדי — AI צריך עובדות, לא תיאורים כלליים.' },
...
[תאר מה קורה]"` },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'text', text: 'הפרומפט לתכנון ראשוני מציג שלוש שאלות מנחות: מסד הנתונים המתאים, מבנה הנתונים, ובעיות אפשריות. שאלות פתוחות אלו מאפשרות ל-AI להציג שיקולים שאולי לא חשבתם עליהם, ולסייע בקבלת החלטות ארכיטקטוניות לפני שמשקיעים זמן בקוד.' },
...
האם יש בעיות שיכולות לצוץ בהמשך?"` },
      ...
      { type: 'text', text: 'ההשוואה בין הגישה הנכונה לרעה ממחישה את ההבדל: ארבע בקשות ממוקדות לעומת בקשה אחת עמוסה. גישת "הכל בבת אחת" מייצרת קוד ארוך שקשה לאתר בו שגיאות, ואם AI יטעה — קשה לדעת היכן. בנייה שלב אחר שלב מאפשרת לאתר ולתקן בעיות מוקדם.' },
...
תאריכים, פרויוריטי, עריכה, מחיקה, sync..."` },
      ...
      { type: 'text', text: 'AI יכול לבצע code review מקיף — הוא לא עייף, לא מוסה ולא חסר סבלנות. התבנית כוללת ארבעה אספקטים: ביצועים, אבטחה, פישוט, ו-edge cases לא מטופלים. כשנותנים רשימה ברורה, AI ממוקד יותר ומעניק תוצאות טובות יותר מאשר "תבדוק את הקוד".' },
...
[הדבק קוד]"` },
      ...
      { type: 'text', text: 'AI מצוין בכתיבת תיעוד — משימה שמפתחים נוטים לדחות. התבנית מגדירה חמישה חלקים ל-README: תיאור קצר, דרישות, התקנה, שימוש, ו-API reference. כשמספקים לAI את מבנה הפרויקט או הקוד, הוא יוצר תיעוד מדויק ולא גנרי.' },
...
[הדבק קוד / מבנה פרויקט]"` },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'text', text: 'השיחה המדגמית מציגה דיבאגינג אמיתי: פונקציה filterByDate שמחזירה רשימה ריקה למרות קלט תקין. הבעיה היא השוואת Date object ל-string — שגיאה קלאסית ש-AI מזהה מיד כשמספקים קלט ופלט. שימו לב שהמשתמש ציין את הקלט המדויק ({date:"2024-01-15"}) ולא רק "לא עובד".' },
...
Claude: "הבעיה היא שהשוואת strings. item.date ו-date
      ...
      { type: 'text', text: 'ארבעה פרומפטים לrefactoring עם מטרות שונות: פישוט ללא שינוי פונקציונליות, חלוקה לפונקציות, הוספת TypeScript types לקוד JS, ושיפור ביצועים. שימו לב לניסוח "ללא שינוי פונקציונליות" — זה מבהיר ל-AI שהמטרה היא קריאות בלבד, לא הוספת פיצ\'רים.' },
...
"תמצא מה מאט את הקוד הזה ותשפר: [קוד]"` },
      ...
      { type: 'text', text: 'ארבע אסטרטגיות מדורגות להתמודדות עם AI שטועה: תיקון ישיר עם השגיאה, הצעת גישה אחרת, בקשה להסביר את הקוד (לאתר אם הבין את הבעיה), ופתיחת שיחה חדשה. הטריק של "תסביר לי שלב שלב" חשוב במיוחד — אם ה-AI מסביר משהו שגוי, הבנתם שלא הבין ותוכלו לנסח מחדש.' },
...
4. לפתוח שיחה חדשה ולנסח מחדש` },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'text', text: 'הדוגמה מציגה ארבעה סוגי פרומפטים לבדיקות: unit tests מלאים עם Jest (happy path + edge cases + error cases), שאילתת edge cases לפונקציה ספציפית, יצירת mock data מציאותי לפי schema, ו-integration test ל-endpoint. שימו לב לפרומפט "מה ה-edge cases" — AI לעתים מוצא בעיות שלא חשבתם עליהן.' },
...
POST /api/users שמקבל {name, email} [קוד route]"` },
      ...
      { type: 'text', text: 'TDD Flow עם AI מורכב משלושה שלבים: הגדרת דרישות בשפה טבעית (מה הפונקציה צריכה לעשות), בקשת טסטים על בסיס הדרישות, ובקשת מימוש שעובר את הטסטים. הדוגמה עם calculateDiscount ברורה: שלושה קופונים עם הנחות שונות וטיפול בקופון לא קיים. AI יכתוב טסטים מדויקים לדרישות אלו.' },
...
"תכתוב את הפונקציה כך שתעבור את הטסטים"` },
      ...
      { type: 'text', text: 'לקומפוננטות React, AI יכול לכתוב React Testing Library tests שבודקים rendering, אינטרקציות ו-state. הפרומפט מציין בדיוק מה לבדוק: rendering תקין, לחיצת כפתור, ו-disabled state. ציון שלושת המקרים מבטיח שAI יכסה את הפונקציונליות העיקרית ולא ייצר טסט אחד שרפרפי.' },
...
[קוד קומפוננטה]"` },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'text', text: 'הפרומפט לסקירת אבטחה מפרט חמישה אזורי סיכון. חשוב לבקש סקירה מפורטת כזו לפחות לפני כל release. שימו לב במיוחד ל-"Data validation בצד שרת" — AI לעתים שם validation רק בצד הלקוח, מה שניתן לעקיפה. אימות תמיד צריך להיות גם בשרת.' },
...
[הדבק קוד]"` },
      ...
      { type: 'text', text: 'ארבע מלכודות נפוצות ב-Vibe Coding: Hallucination (המצאת APIs שלא קיימות), Outdated knowledge (כתיבה בגרסה ישנה), Over-engineering (מורכבות מיותרת), ו-Context loss (שכחת החלטות בשיחות ארוכות). לכל מלכודת יש גישה נגדית פשוטה שניתן ליישם מיד.' },
