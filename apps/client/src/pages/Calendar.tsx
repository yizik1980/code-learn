                            ...
                            <div className="flex flex-col gap-0.5 mt-0.5">
                              {tasks.map(task => (
                                <div key={task.id} className="flex items-center gap-0.5 group" style={{ fontSize: 9 }}>
                                  ...
                                  >{task.done ? '✓' : ''}</button>
                                  <span className="flex-1" style={{ color: task.done ? '#a0998c' : '#1c1c2e', textDecoration: task.done ? 'line-through' : 'none', fontWeight: 600, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{task.title}</span>
